...
> **Path:** $path
> **Responsabilidade em uma frase:** Ler e persistir AppSettings e aplicar efeitos globais.
...
|---|---|---|---|
| egisterSettingsHandlers | function | (...) => void | Registra os canais do dominio com defineHandler ou defineEventHandler. |

## Fluxo Interno
- `SETTINGS_GET` retorna `loadSettings()`.
- `SETTINGS_SET` valida `settingsSchema`, salva JSON, chama `setAutoStart` e `rebuildTrayMenu`.

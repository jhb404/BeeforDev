...
> **Path:** $path
> **Responsabilidade em uma frase:** Receber eventos de controle de janela frameless.
...
|---|---|---|---|
| egisterWindowHandlers | function | (...) => void | Registra os canais do dominio com defineHandler ou defineEventHandler. |

## Fluxo Interno
- `WIN_MINIMIZE` minimiza.
- `WIN_MAXIMIZE` alterna maximize/unmaximize.
- `WIN_CLOSE` esconde janela.
- `WIN_SET_ICON` aceita `data:image/*`, cria `nativeImage` e chama `win.setIcon`.
